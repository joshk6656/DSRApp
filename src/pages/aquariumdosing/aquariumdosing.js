 ...
 */
var AquariumdosingPage = (function () {
    function AquariumdosingPage(navCtrl, navParams, dsrData) {
        this.navCtrl = navCtrl;
        this.navParams = navParams;
        this.dsrData = dsrData;
        this.nomeasurements = false;
        this.correctionvalues = false;
        ...
        if (this.aquarium.measurements.length < 1) {
            ...
            if (this.aquarium.measurements.length >= 1) {
                ...
                var lastmeasurement = this.aquarium.measurements[0];
                ...
                for (var id in this.parameters) {
                    ...
                    if (lastmeasurement.hasOwnProperty(parameter)) {
                        // && onebeforelastmeasurement.hasOwnProperty(parameter)) {
                        this.aquarium.dosings[parameter]["status"] = lastmeasurement[parameter + "status"];
                        var dosinggoal = void 0;
                        var diff = void 0;
                        var checkOnTarget = this.dsrData.checkOnTarget(parameter, lastmeasurement[parameter]);
                        ...
                        if (dosinggoal == "down") {
                            ...
                            if (this.dsrData.dosingcorrections[parameter].hasOwnProperty("correctiontype") && this.dsrData.dosingcorrections[parameter]["correctiontype"] == "%") {
                                ...
                                for (var i = 0; i < day; i++) {
                                    var increasepercentage = ((this.dsrData.dosingcorrections[parameter]["correctionfactor"] / 100) + 1);
    ...
    AquariumdosingPage.prototype.ionViewDidLoad = function () {
        console.log('ionViewDidLoad AquariumdosingPage');
    };
    AquariumdosingPage.decorators = [
        { type: Component, args: [{
    ...
    /** @nocollapse */
    AquariumdosingPage.ctorParameters = function () { return [
        { type: NavController, },
        { type: NavParams, },
        { type: DsrDataProvider, },
    ]; };
    return AquariumdosingPage;
}());
export { AquariumdosingPage };
//# sourceMappingURL=aquariumdosing.js.map